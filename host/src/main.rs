...
use methods::{FHE_WORKSHOP_ELF, FHE_WORKSHOP_ID};
...
struct Arguments {
    #[arg(default_value = "100", short, long)]
    /// Number of random votes to be cast
    votes: Option<usize>,
    #[arg(default_value = "10", short, long)]
    /// Number of parties to the shared public key
    parties: Option<usize>,
    #[arg(default_value = "2048", short, long)]
    /// Degree of the polynomial
    degree: Option<usize>,
    #[arg(short, long)]
    /// Plaintext modulus
    plaintext_modulus: Option<u64>,
    #[arg(default_values = ["18014398492704769"], short, long)]
    /// Moduli
...
fn main() {
    ...
    let num_votes: usize = args.votes.unwrap();
    ...
    let num_parties: usize = args.parties.unwrap();
    ...
    let degree: usize = args.degree.unwrap();
    ...
    let plaintext_modulus: u64;
    if args.plaintext_modulus == None {
        plaintext_modulus = match num_votes {
            1..=999 => 1009,
            1000..=9999 => 10007,
            10000..=99999 => 100003,
        ...
        plaintext_modulus = args.plaintext_modulus.unwrap();
    }
    ...
    let moduli: Vec<u64> = args.moduli.unwrap();
    ...
    let crp: CommonRandomPoly = CommonRandomPoly::new(&params, &mut thread_rng()).unwrap();
    ...
    let serialized_encrypted_votes: Vec<Vec<u8>> = encrypted_votes
        .as_ref()
        .unwrap()
        .par_iter()
        .map(|ct| ct.to_bytes())
    ...
    let env = ExecutorEnv::builder()
        .write(&serialized_encrypted_votes)
        .unwrap()
    ...
    let prove_info = prover.prove(env, FHE_WORKSHOP_ELF).unwrap();

    // extract the receipt.
    let receipt = prove_info.receipt;

    // For example:
    let tally_bytes: Vec<u8> = receipt.journal.decode().unwrap();

    let tally: Arc<Ciphertext> = Arc::new(Ciphertext::from_bytes(&tally_bytes, &params).unwrap());

    // The receipt was verified at the end of proving, but the below code is an
    // example of how someone else could verify this receipt.
    receipt.verify(FHE_WORKSHOP_ID).unwrap();
    ...
    let decryption_shares: Result<Vec<DecryptionShare>, _> = parties
        ...
        .collect();
    let pt: Plaintext = decryption_shares.unwrap().into_iter().aggregate().unwrap();
    let tally_vec: Vec<u64> = Vec::<u64>::try_decode(&pt, Encoding::poly()).unwrap();
